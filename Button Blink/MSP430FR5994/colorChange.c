#include <msp430.h>
 ...
 void main(void)
 {
  ...
  P1DIR |= BIT1;                // Sets Pin 2.0 as an output (Green LED)
  ...
  while(1){                           // Creates an infinite loop to run the program
    ...
    switch (LED) {          // Creates a switch statement to chose the LED
      ...
      P1OUT &= ~BIT1;       // Makes sure that the LED on Pin 2.0 is off by &ing it with itself
      break;

      case 1  :
      P1OUT ^= (BIT1);      // Sets the LED on Pin 2.0 to blink
